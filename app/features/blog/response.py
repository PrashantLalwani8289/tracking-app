...
class GPT:
    ...
    def __init__(self):
        ...
        self.api_key = env_data["GEMINI_API_KEY"]

    def query(self, input: str) -> list:
        genai.configure(api_key=self.api_key)
        prompt = f"""
        {{  ** Do not add any special symbols like ```, -, * , etc. in the start of the response **
            "task": "Generate a detailed travel guide for a specific destination based on the provided input. Provide only the details of the input_destination, the below example are for your reference to understand the type of output. And do not add any *'backslash n'* or break lines",
            ...
            {{
                ...
                "accommodationReview": "From luxury resorts in Nusa Dua to budget-friendly hostels in Seminyak, Bali has accommodations to suit all budgets. Consider staying in a traditional Balinese villa for an authentic experience.",
            ...
            {{
                ...
                "accommodationReview": "Choose from charming boutique hotels in Montmartre to luxury suites with Eiffel Tower views. Paris has a wide range of accommodation options to cater to every traveler.",
