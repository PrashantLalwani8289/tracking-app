...
class CreateBlog(BaseModel):
    ...
    tips: Optional[str] = Field(
        None, example="Pack light, always carry a first-aid kit."
    )
    adventure: str = Field(..., example="Hiking in the Alps")
    accomodationReview: Optional[str] = Field(
        None, example="Stayed at a cozy mountain lodge..."
    )
    destinationGuides: Optional[str] = Field(
        None, example="Top 10 things to do in Paris..."
    )
    customerReview: Optional[str] = Field(None, example="The service was excellent!")
    travelChallenges: Optional[str] = Field(
        None, example="Lost luggage at the airport..."
    )
