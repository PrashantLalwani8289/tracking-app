...
async def sub_for_newsletter(request: SubEmail, db: Session):
    try:
        existing_subscriber = (
            db.query(Subscribers).filter(request.email == Subscribers.email).first()
        )
        ...
        #         "success": False,
        #     }
        new_subscriber = Subscribers(email=request.email)
...
async def count_subs(db: Session):
    try:
        sub_count = db.query(Subscribers).count()
        likes = db.query(Like).count()
        return {
            "message": "Subscribers count successfully",
            "success": True,
            "data": {"sub_count": sub_count, "likes": likes},
        }
    except Exception as e:
        print(e)
        return {
